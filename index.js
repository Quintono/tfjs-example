...
function getImagePrompt() {
  return IMAGENET_CLASSES[
    Math.floor(Math.random() * Object.keys(IMAGENET_CLASSES).length - 1)
  ];
}

async function app() {
  document.getElementById("item-to-find").textContent = getImagePrompt();

  const webcamElemnt = document.getElementsByTagName("video")[0];
  const model = await mobilenet.load();
  const webcam = await tf.data.webcam(webcamElemnt);
  ...
  captureButton.onclick = async () => {
    ...
    document.getElementById(
      "prediction"
    ).textContent = `I'm ${prediction.probability}% sure you showed me a ${prediction.className}.`;
  };
}
